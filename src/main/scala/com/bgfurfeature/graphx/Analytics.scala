 ...
 */
object Analytics extends CLogger {
  ...
  def main(args: Array[String]): Unit = {
    ...
    taskType match {
      case "pagerank" =>
        ...
        if (!outFname.isEmpty) {
          warn("Saving pageranks of pages to " + outFname)
      ...
      case "cc" =>
        ...
        val sc = new SparkContext(conf.setAppName("ConnectedComponents(" + fname + ")").setMaster("local"))
        val unpartitionedGraph = GraphLoader.edgeListFile(sc, fname,
          numEdgePartitions = numEPart,
          edgeStorageLevel = edgeStorageLevel,
          vertexStorageLevel = vertexStorageLevel).cache()
        ...
        cc.vertices.map { case (vid, data) => data }.distinct().foreach{ println }
